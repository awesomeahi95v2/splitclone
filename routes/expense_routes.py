...
def add_expense(group_id):
    ...
    if not all([description, amount, paid_by, created_by]):
        return jsonify({"error": "Missing required fields"}), 400

    try:
        expense = ExpenseService.add_expense(description, amount, currency, paid_by, created_by, group_id)
...
def edit_expense(expense_id):
    ...
    try:
        expense = ExpenseService.edit_expense(expense_id, user_id, **data)
