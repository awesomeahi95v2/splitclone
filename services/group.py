...
class GroupService:
    ...
    def settle_up(group_id):
        ...
        members = [m.user_id for m in group.memberships if m.status == "joined"]
        if not members:
            raise ValueError("No joined members in the group")
        
        num_members = len(members)
        balances = defaultdict(float)

        for expense in group.expenses:
            split_amount = float(expense.amount) / num_members
